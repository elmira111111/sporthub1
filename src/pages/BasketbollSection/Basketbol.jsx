    ...
    return (
        <div className="card" style={{ backgroundImage: `url(${background})` }}>
            <img
                src={image}
                alt="icon"
                className={`card-icon ${isClicked ? 'rotating moving' : ''}`} // Добавляем классы для вращения и движения
                style={{ marginLeft: '-40px' }}
            />
            <h2 className="card-title">{title}</h2>
            <button className="card-button" onClick={handleClick}>Открыть</button> {/* Обработчик клика */}
        </div>
    );
};
